struct G {
	...
	double tau = .9; // time constant; TODO: add diff equ for more realistic one
	double a_sym = 0.5; // alpha sym
	double a_asym = -1.5; // alpha asym
};

void ext_input(char direction, double speed, double *gc_firing, double (*gc_weights)[100], G* g);

void move_path(double *gc_firing, double (*gc_weights)[100], int t, G* g) {
	// movement path

	double speed = g->speed;
	if (t == 1) {
		ext_input('u', speed, gc_firing, gc_weights, g);
	}
	else if (t == 3) {
		ext_input('u', speed, gc_firing, gc_weights, g);
	}
		...
		ext_input('r', speed, gc_firing, gc_weights, g);
	}
	else if (t == 9) {
		ext_input('r', speed, gc_firing, gc_weights, g);
	}
		...
		ext_input('l', speed, gc_firing, gc_weights, g);
	}
	else if (t == 17) {
		ext_input('l', speed, gc_firing, gc_weights, g);
	}
		...
		ext_input('d', speed, gc_firing, gc_weights, g);
	}
	else if (t == 23) {
		ext_input('u', speed, gc_firing, gc_weights, g);
	}
